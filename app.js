...
function renderRecipes() {
  ...
  // 1. Find ingredients expiring in 7 days or less
  const expiringIngredients = foods.filter(f => daysLeft(f.expiry) <= 7);
  const ingredientsToUse = expiringIngredients.length > 0 ? expiringIngredients : foods;

  // 2. Define a list of global discovery recipes for users to browse when pantry is low
  const DISCOVER_DATABASE = [
    {
      name: "Japanese Chicken Katsu Curry",
      emoji: "🍛",
      category: "Comfort Food",
      ingredients: ["Chicken Breast", "Curry Roux Cubes", "Potato", "Carrot", "Rice"],
      instructions: "Bread and deep-fry chicken cutlets. Simmer carrots and potatoes in water, melt down the curry roux blocks until thick, and serve over hot rice."
    },
    {
      name: "Classic Italian Margherita Pizza",
      emoji: "🍕",
      category: "Weekend Baking",
      ingredients: ["Pizza Dough", "Tomato Sauce", "Fresh Mozzarella", "Basil Leaves"],
      instructions: "Roll out dough, spread a thin layer of seasoned tomato sauce, top with torn mozzarella chunks, and bake at maximum heat until bubbly."
    },
    {
      name: "Creamy Garlic Butter Salmon",
      emoji: "🐟",
      category: "Quick Gourmet",
      ingredients: ["Salmon Fillet", "Garlic", "Butter", "Heavy Cream", "Lemon"],
      instructions: "Pan-sear salmon in butter until golden. Remove fish, sauté minced garlic in the same pan, pour in cream and lemon juice, reduce, and pour over fish."
    },
    {
      name: "Fluffy American Buttermilk Pancakes",
      emoji: "🥞",
      category: "Breakfast Classics",
      ingredients: ["Flour", "Sugar", "Baking Powder", "Buttermilk", "Egg", "Butter"],
      instructions: "Whisk dry and wet ingredients separately, combine gently (leave small lumps), cook on a greased griddle until bubbles form, then flip."
    }
  ];

  // 3. CASE A: User has zero food items at all — Show ONLY the Discover View
  if (ingredientsToUse.length === 0) {
    const discoverCardsHtml = DISCOVER_DATABASE.map((r, idx) => `
      <div class="recipe-card discover-card">
        <div class="discover-badge">✨ Discover</div>
        <div class="recipe-header">
          <span class="recipe-emoji">${r.emoji}</span>
          <div>
            <h4 class="recipe-name">${r.name}</h4>
            <span class="discover-cat">${r.category}</span>
          </div>
        </div>
        <div class="discover-ingredients">
          <strong>Ingredients needed:</strong>
          <div class="discover-ing-list">
            ${r.ingredients.map(ing => `
              <button class="add-missing-btn" onclick="addMissingToShop('${ing.replace(/'/g, "\\'")}')" title="Add to shopping list">
                ＋ ${ing}
              </button>
            `).join('')}
          </div>
    ...
    container.innerHTML = `
      <div class="discover-hero-banner">
        <div class="empty-icon" style="margin:0; font-size:2.5rem;">📖</div>
        <div>
          <h3>Your pantry is currently empty!</h3>
          <p>Discover these trending meal ideas and tap any ingredient to add it to your 🛒 Shopping List.</p>
        </div>
      </div>
      <div class="recipes-grid">${discoverCardsHtml}</div>
    `;
    return;
  }

  // 4. CASE B: User HAS food items — Show standard matching algorithm
  const availableNames = ingredientsToUse.map(f => (f.name || '').toLowerCase().trim());
  ...
  const RECIPE_DATABASE = [
    {
      ...
      keywords: ["rice", "egg", "carrot", "garlic", "onion", "meat", "chicken", "broccoli", "produce"],
      emoji: "🍛",
      instructions: "Sauté garlic and onions, toss in chopped proteins/veggies, add cooked rice and soy sauce, then scramble an egg right into the pan."
    },
    {
      name: "Rescue-Mission Omelette",
      keywords: ["egg", "cheese", "milk", "tomato", "spinach", "onion", "mushroom", "meat", "dairy"],
      emoji: "🍳",
      instructions: "Whisk eggs with a splash of milk, pour into a hot buttery skillet, and fold in any expiring cheeses, meats, or vegetables."
    },
    {
      name: "Clear-The-Fridge Vegetable Soup",
      keywords: ["carrot", "tomato", "broccoli", "onion", "garlic", "potato", "celery", "cabbage", "produce"],
      emoji: "🥣",
      instructions: "Chop all remaining vegetables. Simmer them in a pot with water or broth and household seasonings until tender."
    },
    {
      name: "Crispy Sheet-Pan Stir Fry / Hash",
      keywords: ["meat", "beef", "chicken", "broccoli", "carrot", "potato", "onion", "produce"],
      emoji: "Ⓥ",
      instructions: "Dice your meat and hardy veggies, toss in oil and spices, and roast on a baking sheet at 200°C until crispy."
    },
    {
      name: "Pantry Pasta Aglio e Olio / Marinara",
      keywords: ["pasta", "spaghetti", "garlic", "tomato", "cheese", "onion"],
  ...
  // Check if any recipe actually matched an ingredient
  const highestMatch = Math.max(...scoredRecipes.map(r => r.matchCount));

  // Sub-case: If user has items but absolutely ZERO matches were generated, insert a helper banner
  let titleHtml = '';
  if (highestMatch === 0) {
    titleHtml = `<div class="recipe-status-alert fallback">ℹ️ No exact cooking matches found for your current items. Here are universal foundational dishes:</div>`;
  } else if (expiringIngredients.length > 0) {
    titleHtml = `<div class="recipe-status-alert">⚠️ Prioritizing recipes using <strong>${expiringIngredients.length} expiring item(s)</strong>!</div>`;
  } else {
    titleHtml = `<div class="recipe-status-alert fresh">💡 Everything is fresh! Here are recipes matching your general pantry:</div>`;
  }
  ...
  const cardsHtml = scoredRecipes.map(r => {
    const matchTags = r.matchCount > 0 
      ? `<div class="recipe-matches">✨ Uses your: ${r.matchedIngredients.map(m => `<span class="match-tag">${m}</span>`).join('')}</div>`
      : `<div class="recipe-matches none">No direct matches, but great for household staples!</div>`;
...
// ── Global Helper: Click discovery ingredient to send to shopping list ──
function addMissingToShop(itemName) {
  if (!itemName) return;
  shopItems.push({ name: itemName, checked: false });
  saveShopItems();
  toast(`🛒 Added "${itemName}" to Shopping List!`);
}
